...
const Pricing = () => {
  ...
  const [pending, setPending] = useState<{ priceId: string; planName: string } | null>(null);
  ...
  const handleSubscribe = async (priceId: string, planName: string) => {
    ...
    // Require authentication before creating checkout session
    if (!isAuthenticated) {
    ...
    try {
      const { url } = await stripeAPI.createCheckoutSession({ priceId, planName });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <SignInModal
        ...
        initialTab={pending ? 'signup' : 'signin'}
        onSuccess={async () => {
          if (pending) {
            const { priceId, planName } = pending;
            setPending(null);
            setIsLoading(priceId);
            try {
              const { url } = await stripeAPI.createCheckoutSession({ priceId, planName });
